...
object Vector {
  @mutable def apply[T:Manifest](__arg0: Int): Vector[T] = /*allocates*/  new Vector[T](__arg0,array_empty[T](__arg0))

  protected def vector_raw_data[T:Manifest](self: Vector[T]): ForgeArray[T] = /*getter*/  self._data

  protected def vector_set_raw_data[T:Manifest](@write self: Vector[T],__arg1: ForgeArray[T]): Unit = /*setter*/  self._data = __arg1

  protected def vector_set_length[T:Manifest](@write self: Vector[T],__arg1: Int): Unit = /*setter*/  self._length = __arg1

  protected def vector_insertspace[T:Manifest](@write self: Vector[T],pos: Int,len: Int): Unit = Delite.single({
  ...
  protected def vector_ensureextra[T:Manifest](@write self: Vector[T],extra: Int): Unit = Delite.single({
      val data = vector_raw_data(self)
  ...
  protected def vector_realloc[T:Manifest](@write self: Vector[T],minLen: Int): Unit = Delite.single({
      val data = vector_raw_data(self)
  ...
  protected def vector_copy[T:Manifest](self: Vector[T],__arg1: Int,@write __arg2: Vector[T],__arg3: Int,__arg4: Int): Unit = Delite.single({
...
class Vector[T:Manifest](___length: Int, ___data: ForgeArray[T]) extends  ParallelCollectionBuffer[T]  { self => 
  ...
  import Vector._
  def length(): Int = Forge.getter(self, _length)

  def apply(__arg0: Int): T = Delite.composite(array_apply(vector_raw_data(self), __arg0))

  @write def update(i: Int,e: T): Unit = Delite.composite(array_update(vector_raw_data(self), i, e))
  ...
  @write def append(__arg0: Int,__arg1: T): Unit = Delite.single(self.insert(self.length, __arg1))
  ...
  @simple def pprint(): Unit = Delite.foreach[T](self, (a => println(a)))
